...
class NeuralSurvivalClusterTorch(nn.Module):
  ...
  def __init__(self, inputdim, layers = [50], act = 'Tanh', layers_surv = [50], representation = 2, act_surv = 'Tanh',
...
class NeuralSurvivalClusterTorchJ(nn.Module):
  def __init__(self, inputdim, layers=[50], act='Tanh', layers_surv=[50], representation=2,
               act_surv='Tanh',
               weight_balance=1., risks=1, k=2, dropout=0., optimizer="Adam"):
    super(NeuralSurvivalClusterTorchJ, self).__init__()
    self.input_dim = inputdim
    self.weight_balance = weight_balance  # Used for balancing the loss between censored and uncensored
    self.risks = risks  # Competing risks
    self.k = k  # Number mixture
    ...
    self.outcome = nn.ModuleList(
      [create_representation_positive(1 + self.representation, layers_surv + [risks], act_surv, self.dropout) for _ in
       range(self.k)])
    self.competing = create_representation(inputdim, layers + [risks], act, self.dropout)
    self.soft = nn.Softmax(dim=1)
  ...
  def forward(self, x, horizon, gradient=False):
    ...
    cumulative = torch.cat(cumulative, -1)
    intensity = torch.cat(intensity, -1) if gradient else None
    # print(cumulative.shape, intensity.shape)
    return cumulative, intensity

  def predict(self, x, alphas, horizon):
    cumulative, _ = self.forward(x, torch.FloatTensor(horizon.float()))
    # alphas = nn.Softmax(dim=1)(alphas).unsqueeze(1).repeat(1, self.risks, 1)  # Repeat alpha for each risk
    return torch.sum(alphas * torch.exp(-cumulative), dim=2), torch.exp(-cumulative)